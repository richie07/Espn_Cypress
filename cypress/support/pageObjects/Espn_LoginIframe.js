...
class Espn_LoginIframe{
    ...
    _doLoginInPage(){
        cy.fixture("dataUser").then((dataJson)=>{
            cy.log(`Login with Name :${dataJson.first_name} and Password ${dataJson.password}`)
    ...
    _deleteAccount(){
        ...
        cy.document().then((doc) => {
            ...
            cy.get('@iframe').find(this.btnDeleteAccount).scrollIntoView({ easing: 'linear' }).should('be.visible').wait(1000).click({force:true})
          });
    ...
    _validateDeleteAccount(){
        cy.get('@iframe').find(this.lblTitle).should('contain.text','Your account has been deleted.')
    }

}
export default Espn_LoginIframe